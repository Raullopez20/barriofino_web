import"./hoisted.B8OeWZTR.js";document.addEventListener("DOMContentLoaded",async()=>{document.getElementById("upcoming-events-container");const s=document.getElementById("events-loading"),r=document.getElementById("no-events"),a=document.getElementById("events-grid");try{const o=await fetch("/barriofino_web/data/events.json");if(!o.ok)throw new Error("No se pudieron cargar los eventos");const c=await o.json(),m=new Date,i=c.filter(e=>new Date(e.date+"T"+(e.time||"00:00"))>=m);i.sort((e,t)=>{const d=new Date(e.date+"T"+(e.time||"00:00")),n=new Date(t.date+"T"+(t.time||"00:00"));return d-n});const l=i.slice(0,3);s.style.display="none",l.length===0?r.classList.remove("hidden"):(a.classList.remove("hidden"),a.innerHTML=l.map((e,t)=>{const n=new Date(e.date).toLocaleDateString("es-ES",{weekday:"long",year:"numeric",month:"long",day:"numeric"});return`
